...
*			Replace -> EAZY_LCD_I2C_HANDLER 
*					with your own I2C handler
*		For Anyother MCU:
*			Remove/Replace -> extern I2C_HandleTypeDef EAZY_LCD_I2C_HANDLER;
...
*					with your own I2C handler
*		For Anyother MCU:
*			Replace -> HAL_I2C_Master_Transmit(&EAZY_LCD_I2C_HANDLER, (uint16_t)I2C_RGB_ADDR, (uint8_t*)data, 2, 1000);
...
*/
/*************************************************************************/
#if defined(GROVE_LCD_BUS_TYPE_I2C)
#define I2C_RGB_ADDR  0xC4
#define I2C_LCD_ADDR  0x7C
extern I2C_HandleTypeDef EAZY_LCD_I2C_HANDLER;
#define ez_lcd_send_byteS i2c_send_byteS
#endif


static void EAZY_LCD_setReg(unsigned char reg, unsigned char val)
{
  uint8_t data[2] = {reg, val};
  HAL_I2C_Master_Transmit(&EAZY_LCD_I2C_HANDLER, (uint16_t)I2C_RGB_ADDR, (uint8_t*)data, 2, 1000);
}
...
*					with your own I2C handler
*		For Anyother MCU:
*			Replace -> HAL_I2C_Master_Transmit(&EAZY_LCD_I2C_HANDLER, (uint16_t)I2C_LCD_ADDR, (uint8_t*)data, len, 1000);
...
{
  HAL_I2C_Master_Transmit(&EAZY_LCD_I2C_HANDLER, (uint16_t)I2C_LCD_ADDR, (uint8_t*)data, len, 1000);
}
#endif
...
void EAZY_LCD_begin(uint8_t cols, uint8_t lines, uint8_t dotsize)
{
  if (lines > 1) 
  {
  ...
  EAZY_LCD_command(LCD_FUNCTIONSET | _displayfunction);
  ...
  // set the entry mode
  EAZY_LCD_command(LCD_ENTRYMODESET | _displaymode);
  
  // backlight init
  EAZY_LCD_setReg(0, 0);
  EAZY_LCD_setReg(1, 0);
  EAZY_LCD_setReg(0x08, 0xAA);     // all led control by pwm
  
  EAZY_LCD_setColorWhite();
}

void EAZY_LCD_clear(void)
{
  EAZY_LCD_command(LCD_CLEARDISPLAY);        // clear display, set cursor position to zero
  HAL_Delay(2); // this command takes a long time!
}

void EAZY_LCD_home(void)
{
  EAZY_LCD_command(LCD_RETURNHOME);        // clear display, set cursor position to zero
  HAL_Delay(2); // this command takes a long time!
}
...
void EAZY_LCD_noDisplay(void)
{
  ...
  EAZY_LCD_command(LCD_DISPLAYCONTROL | _displaycontrol);
}
...
{
  EAZY_LCD_command(LCD_CURSORSHIFT | LCD_DISPLAYMOVE | LCD_MOVELEFT);
}

void EAZY_LCD_scrollDisplayRight(void)
{
  EAZY_LCD_command(LCD_CURSORSHIFT | LCD_DISPLAYMOVE | LCD_MOVERIGHT);
}
...
void EAZY_LCD_createChar(uint8_t location, uint8_t charmap[])
{
  ...
  EAZY_LCD_command(LCD_SETCGRAMADDR | (location << 3));
...
// color control
void EAZY_LCD_setRGB(unsigned char r, unsigned char g, unsigned char b)
{
  EAZY_LCD_setReg(REG_RED, r);
  EAZY_LCD_setReg(REG_GREEN, g);
  EAZY_LCD_setReg(REG_BLUE, b);
}

void EAZY_LCD_setPWM(unsigned char color, unsigned char pwm)
{
  EAZY_LCD_setReg(color, pwm);
}
...
{
  if(color > 3)return ;
  EAZY_LCD_setRGB(color_define[color][0], color_define[color][1], color_define[color][2]);
}

void EAZY_LCD_setColorAll(void)
{
  EAZY_LCD_setRGB(0, 0, 0);
}

void EAZY_LCD_setColorWhite(void)
{
  EAZY_LCD_setRGB(255, 255, 255);
}
