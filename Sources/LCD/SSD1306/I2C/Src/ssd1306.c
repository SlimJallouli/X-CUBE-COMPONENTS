...
extern I2C_HandleTypeDef SSD1306_I2C_HANDLER;
...
HAL_StatusTypeDef SSD1306_Init(void)
{
  ...
  err = HAL_I2C_IsDeviceReady(&SSD1306_I2C_HANDLER, SSD1306_I2C_ADDR, 1, 20000);
...
static void ssd1306_I2C_WriteMulti(uint8_t address, uint8_t reg, uint8_t* data, uint16_t count) 
{
  ...
  for(i = 1; i <= count; i++)
    dt[i] = data[i-1];
  HAL_I2C_Master_Transmit(&SSD1306_I2C_HANDLER, address, dt, count+1, 100);
}
...
static void ssd1306_I2C_Write(uint8_t address, uint8_t reg, uint8_t data)
{
  ...
  dt[0] = reg;
  dt[1] = data;
  HAL_I2C_Master_Transmit(&SSD1306_I2C_HANDLER, address, dt, 2, 100);
}
/* USER CODE END */
